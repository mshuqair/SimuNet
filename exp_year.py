...
# The data generated using a first exponential function
...
a_s = np.array([4, 4, 4, 4, 4])
b_s = np.array([2160, 2097.087, 2036.007, 1976.706, 1919.132])
...
for key, year in zip(data.keys(), years):
    ...
    data[key]['Unplanned Cost'] = a_s[year - 1] * np.exp(-a_s[year - 1]*data[key]['Maintenance No.']) + b_s[year - 1]
...
if not os.path.isfile('models/model_nn_exp_years.h5'):
    model_nn = build_model()
    callback_stopping = EarlyStopping(monitor='loss', patience=5)
    history = model_nn.fit(x_train, y_train,
                           batch_size=batch_size, epochs=epochs,
                           callbacks=[callback_stopping])
    model_nn.save('models/model_nn_exp_years.h5')
else:
    model_nn = load_model('models/model_nn_exp_years.h5')
