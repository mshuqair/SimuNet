...
if not os.path.isfile('models/model_nn_linear_split.h5'):
    model_nn = build_model()
    callback_stopping = EarlyStopping(monitor='loss', patience=5)
    history = model_nn.fit(x_train, y_train,
                           batch_size=batch_size, epochs=epochs,
                           callbacks=[callback_stopping])
    model_nn.save('models/model_nn_linear_split.h5')
else:
    model_nn = load_model('models/model_nn_linear_split.h5')
...
for year, year_index in zip(years, years-1):
    ...
    _, ax = plt.subplots(nrows=1, ncols=1, figsize=(6.5, 3), layout='constrained')
